...
public class SpaceWandererExample {
    ...
    public static void main (String[] args) {
        ...
        while (true) {
            ...
            System.out.println (decodeSonars (rngi));
    ...
    static String align (NumberFormat fmt, double n, int sp) {
    ...
    public static String decodeSonars (RangerInterface rngi) {
        String out = "\nSonar vars: \n";
        for (int i = 0; i < rngi.getData ().getRanges_count (); i++) {
            out += " [" + align (fmt, i+1, 2) + "] = " + 
            align(fmt, rngi.getData ().getRanges ()[i], 5);
            if (((i+1) % 8) == 0)
                out += "\n";
        }
        return out;
