 ...
 *
 * $Id: HeadingControl.java 47 2006-03-06 08:33:31Z veedee $
 *
 */
package javaclient2.extra;

import javaclient2.AbstractPositionDevice;
...
public class HeadingControl extends PIDController {
	...
	private float minCommand = 1;
	private float maxCommand = 180;
	 ...
	 */
	private float boundCommand (float command) {
		if (command == 0)
			return 0;
		if (command < 0) {
	...
	public boolean setDiffHeading (float angle) {
		...
		float currentHead = transformAngle (device.getYaw ());
		/* calculate the goal heading */
		float newGoal     = transformAngle (currentHead + angle);
		
		setGoal (newGoal);
		
		float now = transformAngle (device.getYaw ());
